...
var (
	...
	DevsidhAPI           = os.Getenv("DEVSIDH_API")
	DevsidhAPIToken      = os.Getenv("DEVSIDH_API_TOKEN")
	DevsidhAPIPassword   = os.Getenv("DEVSIDH_API_PASSWORD")
	DevsidhAPIUsername   = os.Getenv("DEVSIDH_API_USERNAME")
	BoompayAPI           = os.Getenv("BOOMPAY_API")
	BoompayClientID      = os.Getenv("BOOMPAY_CLIENT_ID")
	BoompayAPIToken      = os.Getenv("BOOMPAY_API_TOKEN")
	BoompayAPISecret     = os.Getenv("BOOMPAY_API_SECRET")
)
...
const (
	...
	BoompayPayout                      = "/payout/w/ur/initiate-payout-request"
	BoompayPayoutWalletBalance         = "/merchant/get/wallet-balance"
	BoompayPayoutStatusCheck           = "/payout/w/ur/initiated-payout-status"
	BoompayPayoutAccessTokenGeneration = "/auth/generate-token"
	BoompayPennyDrop                   = "/bank/verification-penny-drop"
)
...
func PostRequest4(url, authKey1, authValue1, authKey2, authValue2, authKey3, authValue3, authKey4, authValue4 string, body map[string]any, res any) error {
	b, err := json.Marshal(body)
	if err != nil {
		return fmt.Errorf("PostRequest marshal: %w", err)
	}
...
func GetRequest3(url, authKey1, authValue1, authKey2, authValue2, authKey3, authValue3 string, res any) error {
	req, err := http.NewRequest(http.MethodGet, url, nil)
	if err != nil {
		return fmt.Errorf("GetRequest build: %w", err)
	}
...
func GetRequest4(url, authKey1, authValue1, authKey2, authValue2, authKey3, authValue3, authKey4, authValue4 string, res any) error {
	req, err := http.NewRequest(http.MethodGet, url, nil)
	if err != nil {
		return fmt.Errorf("GetRequest build: %w", err)
	}
